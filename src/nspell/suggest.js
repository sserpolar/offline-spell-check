/*
 * nspell suggestion engine — ESM port
 *
 * ⚠️ 移植代码。上游 nspell 2.1.5 (MIT) 的 `lib/suggest.js`,逐行照搬。
 * 改动:仅模块语法。算法一字未改。
 *
 * ⏱️ 成本提示:`correct()` 是 0.000ms 级(实测 1000 次 < 1ms),
 *    但 `suggest()` **贵得多** —— 它要生成编辑距离 1(必要时 2)的全部候选词
 *    再逐个查表。所以产品里 suggest **只按需调用**:
 *    用户点了某个红色标记 / 点了 popup 列表里的某个词,才发一次 SUGGEST 消息。
 *    绝不在扫描时给所有错词预先算建议。
 */

import { casing, normalize, flag } from './form.js';
import form from './form.js';

var push = [].push;

// Suggest spelling for `value`.
export default function suggest(value) {
  var self = this;
  var charAdded = {};
  var suggestions = [];
  var weighted = {};
